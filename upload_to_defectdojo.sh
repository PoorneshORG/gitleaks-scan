...
# - Product   = DevSecOps (fixed)
# - Engagement = Gitleaks (fixed, must exist already)
# - Test      = Repository name
# - Findings  = repo-branch-rule
#
# Rules:
# 1. If no secrets and no test exist yet → do nothing.
# 2. If secrets found for the first time → create test (repo name).
# 3. If secrets found later in same repo → reimport into existing test.
# 4. Branch name only affects finding titles, not test creation.
# 5. If no secrets but test already exists → reimport empty scan to close findings.
# -------------------------
...
PRODUCT_NAME=$3
SCAN_FILE=$4
REPO_NAME=$5       # Test title (repo)
BRANCH_NAME=$6     # Used only for finding titles
...
# -------------------------
# Step 3: Engagement must exist already
# -------------------------
...
if [ -z "$ENGAGEMENT_ID" ] || [ "$ENGAGEMENT_ID" == "null" ]; then
  echo "❌ Engagement '$ENGAGEMENT_NAME' not found in DefectDojo. Please create it manually."
  exit 1
fi

# -------------------------
# Step 4: Test = Repo name
# -------------------------
TEST_TITLE="$REPO_NAME"
...
if [ -z "$TEST_ID" ] || [ "$TEST_ID" == "null" ]; then
  if [ "$SECRETS_FOUND" -eq 0 ]; then
    echo "✅ No findings found. Skipping upload for new repo '$TEST_TITLE'."
    exit 0
  fi
  echo "🧪 Test '$TEST_TITLE' will be created by first reimport."
else
  if [ "$SECRETS_FOUND" -eq 0 ]; then
    echo "✅ No findings found. Reimporting empty scan to close findings for '$TEST_TITLE'."
  fi
fi
